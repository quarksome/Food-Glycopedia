...
def json_abs_data():
    ...
    df = df.dropna(subset=['UID','all_descriptors'])
...
def foodgroup_filter_plot(df, monoList, food_group_list):
    """
    Function to filter dataframe according to selected mono and food_group
    """
    temp = df[df['food_group'].str.contains('|'.join(food_group_list), na=False)]
...
#get all values in food_group for the Dropdown list
df = pd.read_json(json_abs_data(), orient='split')
food_grp_list = df['food_group'].unique().tolist()
food_group_options = [{'label':x, 'value':x} for x in food_grp_list if x not in [None, '#N/A']]
...
app.layout = html.Div(style={'backgroundColor': colors['background']}, children=[
    ...
    dcc.RadioItems(
        ...
        {'label': 'food group', 'value': 'food_group'}
        ],
        value='keyword',
        labelStyle={'display': 'inline-block'}
    ),
    ...
    html.Label('Choose a food group to display'
    ),
...
@app.callback(
    [Output('filtered-data-holder', 'children'),
     ...
     Output('filter-word', 'disabled'),
     Output('food-group-filter', 'disabled')],
...
def data_process(absData, relData, quant_option, monoList, filterWord, filter_option, food_group_list):
    """
    ...
    """
    if filter_option=='keyword':
        foodgroup_state = True
        keyword_state = False
    ...
    if filter_option=='food_group':
        keyword_state = True
        foodgroup_state = False
        
        if quant_option=='absolute':
            df = pd.read_json(absData, orient='split')
            temp_abs_json, fig = foodgroup_filter_plot(df, monoList, food_group_list)
            temp_rel_json = pd.DataFrame().to_json(orient='split')
            fig.update_layout(
                xaxis=dict(
                    title='concentration (mg/mg)')
            ...
            df = pd.read_json(relData, orient='split')
            temp_rel_json, fig = foodgroup_filter_plot(df, monoList, food_group_list)
            temp_abs_json = pd.DataFrame().to_json(orient='split')
            fig.update_layout(
                xaxis=dict(
                    title='fractional relative abundance')
    ...
    return temp_abs_json, temp_rel_json, fig, keyword_state, foodgroup_state
